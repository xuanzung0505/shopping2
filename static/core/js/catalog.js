...
function clean(data) { //get json by defaul serialize
    ...
    jQuery(data).each(function (i, item) {
        ...
        var aNewData = {
            ...
            "totalStar": item.fields.totalStar, "reviews": JSON.parse(item.fields.reviews)
        }
...
function appendData(pk, title, unitPrice, imgPath, totalReview, totalStar) {
    $("#itemList").append(
        ...
        imgPath + '"></a>' +
        '</div>' +
        '<div class="product-name"><a href="' + "/detail/" + pk + '">' + title + '</a></div>' +
...
function renderData() {
    ...
    for (var i = (current_page - 1) * records_per_page; (i <= current_page * records_per_page - 1)
        && i < pageData.length; i++) {
        var item = pageData[i]
        var pk, title, unitPrice, imgPath, totalReview, totalStar;
        ...
        appendData(pk, title, unitPrice, imgPath, totalReview, totalStar)
    }
}
...
$(document).ready(function () {
    console.log("client catalog page")
