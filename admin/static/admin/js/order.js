...
function appendData(id, totalPrice, create_at, active, status, user) {
    var statusSection = ''
    ...
    $("#orderList").append(
        ...
        '<a class="product-name" href="/admin/order/' + id + '">Đơn hàng #' + id + '</a>' +
        '<br>' +
        '<a class="product-name" href="/admin/user/' + user.id + '">' + user.username + '</a>' +
        '<br>' +
        formattedDate +
        '</div>' +
...
function renderData() {
    ...
    for (var i = (current_page - 1) * records_per_page; (i <= current_page * records_per_page - 1)
        ...
        var id, totalPrice, create_at, active, status, user;
